...
<script>
  export default {
    ...
    methods: {
      async login() {
        this.$tokenAuth.login({data:this.model}).then((res)=>{
          console.log(this.$tokenAuth.getToken())
          // this.$axios.setHeader('Authorization', 'Token '+this.$tokenAuth.getToken())
          // this.$axios.setToken(this.$tokenAuth.getToken(),'',['get','post']);
          // console.log(this.$axios.setToken(res.data.token,'',['get','post']))
          this.$router.push({
            name: 'dashboard'
          })
        })
